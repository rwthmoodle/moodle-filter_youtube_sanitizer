...
class filter_youtube_sanitizer extends moodle_text_filter {
    ...
    public function filter($text, array $options = []) {
        ...
        $text = preg_replace_callback('~(<span class="mediaplugin mediaplugin_youtube">[^<]*)?<iframe[^>]* src="((:?https?://)?(:?(:?www|m)\\.)?(:?youtube\\.com|youtube-nocookie\\.com|youtu\\.be)[^"]*)"[^>]*>[^<]*</iframe>(?(1)[^<]*</span>)~', function($matches) use ($cache) {
            $videourl = $matches[2];
